...
public class CountryList {
    ...
    public Country getLast() {
    	int last = countryList.size() -1;
    	return countryList.get(last);
    }
    
    public Country getPopMedian() {
    	ArrayList<Country> myList = countryList;
    	myList.sort(Comparator.comparing(Country::getPopulation)); // Java 8
    	int mid = myList.size() / 2;
    	if ((mid % 2) == 0){
    		return myList.get(mid + 1);
    	} else {
    		return myList.get(mid);
    	}
    }
